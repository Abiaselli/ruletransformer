...
def worker_init_fn(worker_id):
    global global_tokenizer
    tokenizer_path = r"/home/austin/vccode/ruletransformer-main2/models/modeltest"
...
def collate_fn_global(batch):
    ...
    if isinstance(batch[0], tuple):
        ...
        for query, target in batch:
            ...
            if len(query_ids) > max_len_q:
                max_len_q = len(query_ids)
            target_ids = global_tokenizer.encode(target, add_special_tokens=False)
            if len(target_ids) > max_len_t:
                max_len_t = len(target_ids)
            query_ids_tensor.append(torch.tensor(query_ids, dtype=torch.long))
            target_ids_tensor.append(torch.tensor(target_ids, dtype=torch.long))
        if max_len_t > max_len_q:
            max_len = max_len_t
        else:
            max_len = max_len_q
        query_ids_tensor = pad_sequences(query_ids_tensor, max_len, pad_value=PAD_ID)
        target_ids_tensor = pad_sequences(target_ids_tensor, max_len, pad_value=PAD_ID)
        # Pad the sequences to the same length
        return query_ids_tensor, target_ids_tensor
    ...
    def training_loop(self):
        ...
        try:
            ...
            for epoch in range(self.epochs.get()):
                ...
                # Training loop
                for batch_idx, (batch_input_ids, target_labels) in enumerate(dataloader):
                    if self.stop_training.is_set():
                            logging.info("Training stopped by user.")
                            messagebox.showinfo("Info", "Training stopped by user.")
                            return
                    ...
                    max_len_input = batch_input_ids.shape[1]
                    max_len_target = target_labels.shape[1]
                    #print(f"max_len_input: {max_len_input}, max_len_target: {max_len_target}")
                    if max_len_input > max_len_target:
                    ...
                    # Pad all sequences
                    batch_input_ids = pad_sequences(batch_input_ids, max_len, pad_token_id)
                    target_labels = pad_sequences(target_labels, max_len , pad_token_id)
                    logging.debug(f'Batch input_ids shape after padding: {batch_input_ids.shape}')  # (batch_size, seq_len)
                        # Log the shape of the combined mask
                    logging.debug(f'Target labels shape after padding: {target_labels.shape}')  # (batch_size, seq_len)
                    architecture = self.architecture.get()
                    step_count = 0
                    ...
                    for t in range(2, max_len):
                        ...
                        batch_input_ids = batch_input_ids[:, start:t]                # decoder input
                        target_labels = target_labels[:, start + 1:t + 1]    # target input
                        logging.debug(f'tgt_ids: {target_labels}')  # (batch_size, seq_len)

                        # Clip if lengths don’t match due to short edges
                        min_len = min(batch_input_ids.size(1), target_labels.size(1))
                        batch_input_ids = batch_input_ids[:, -min_len:]
                        target_labels = target_labels[:, -min_len:]
                        logging.debug(f'tgt_ids after adjustment: {target_labels}')  # (batch_size, seq_len)

                        if batch_input_ids.size(1) == 0 or target_labels.size(1) == 0:
                            continue
                        ...
                        with torch.amp.autocast(device, dtype=torch.float16):  # Enable mixed precision
                            ...
                            if self.use_genetic_algo == "Genetic Algorithm":
                                    ...
                                    self.model = qga.evolve(loss_fn, batch_input_ids, target_labels, target_labels, architecture)
                                ...
                                try:
                                    if architecture == "Rule Transformer":
                                        ...
                                        output = checkpoint.checkpoint(self.model, batch_input_ids, target_labels, use_reentrant=False)
                                except Exception as e:
                                ...
                                if architecture == "Rule Transformer":
                                    ...
                                    loss = loss_fn(output_filtered, target_labels_reshaped)
                                ...
                                n+=1
                                print(f"Iteration {n}, step: {step_count} Loss: {loss.item()}")
                                ...
                                if architecture == "Rule Transformer":
                                        ...
                                        try:
                                            with torch.no_grad():
                                                output_new = self.model(batch_input_ids, target_labels)
                                        except Exception as e:
                                            traceback.print_exc()
                                            logging.error(f"Model rule validation failed: {e}")
                                        #output_new = model(src)
                                        logging.debug("Rule output generated")
                                        new_loss = loss_fn(output_new.reshape(-1, output_new.shape[-1]), 
                                                    target_labels.reshape(-1)).item()
                                        logging.debug("new loss called")
