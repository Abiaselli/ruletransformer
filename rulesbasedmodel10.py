...
class ReasoningModelGUI:
    ...
    def load_trained_model(self):
        ...
        if os.path.exists(trained_model_path):
            self.model.load_state_dict(torch.load(trained_model_path), strict=False)  # Load updated model
            # Load model state
            if self.architecture == "Rule Transformer":
                # 🔹 Extract the number of rules from the saved model
                self.num_saved_rules = self.model.state_dict["embedding.rule_transform"].shape[0]
    ...
    def start_training(self):
        ...
        if self.training_process is not None:
            ...
            return        # Start training in a separate thread to keep the GUI responsive
        ...
        def run_training():
            self.training_process = subprocess.Popen(["torchrun", "--nnodes=1", "--nproc_per_node=2", "training_loop4.py", model_path, config_path])

            print(f"✅ Training started with PID: {self.training_process.pid}")
            self.training_process.wait()  # Wait for training to finish
            self.training_process = None  # Reset when training stops

        training_thread = threading.Thread(target=run_training)
        training_thread.start()
        # Load updated model after training
        self.load_trained_model()
        
    def stop_training_command(self):
        """Stops training and loads the saved model."""
        if self.training_process is not None:
            print("🛑 Stopping training...")
            with open('stop_training.txt', 'w', encoding='utf-8') as f:
                f.write('\n')
            self.training_process.wait()  # Ensure cleanup
            self.training_process = None  # Reset process variable
            print("✅ Training stopped.")

            # ✅ Load the saved model into the GUI
            trained_model_path = "trained_model.pth"
            if os.path.exists(trained_model_path):
                self.load_trained_model()
                print("✅ Loaded updated model into GUI.")
            ...
            print("⚠️ No trained model found. Training may not have saved.")
