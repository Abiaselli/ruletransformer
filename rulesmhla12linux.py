...
def collate_fn_global(batch):
    ...
    if isinstance(batch[0], str):
        ...
        input_ids = pad_sequence(input_ids, batch_first=True, padding_value=PAD_ID)  # [batch, max_len]
        target_ids = encode_and_fix(targets)
        target_ids = pad_sequence(target_ids, batch_first=True, padding_value=PAD_ID)  # [batch, max_len]
            ...
            for epoch in range(self.epochs.get()):
                ...
                for batch_idx, (batch_input_ids, target_labels) in enumerate(dataloader):
                    ...
                    if max_len_input > max_len_target:
                        ...
                        del max_len_input, max_len_target
                            ...
                            if self.use_genetic_algo == "Genetic Algorithm":
                                ...
                                if (batch_idx + 1) % 10 == 0:     
                                        if architecture == "Rule Transformer":
                                            ...
                                            logging.debug("Rule optimized and replaced")
                        gc.collect()
                        torch.cuda.empty_cache()
