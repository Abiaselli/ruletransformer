...
def cleanup(model, backup_model_path="backup_model"):
    """Saves the model and cleans DDP."""
    save_model(model, backup_model_path)
    print(f"✅ Model saved to {backup_model_path}")
...
class Rule_Transformer_Model(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads, seq_length, lambda_decay=0.01, memory_size=16, dropout=0.1, compression_factor=4, num_frequencies=100, max_rules=set_number_rules):
        ...
        self.encoder_layers = nn.ModuleList([
            HierarchicalMultiHeadLatentAttention(embedding_dim, num_heads, self.d_latent, lambda_decay, memory_size)
    ...
    def forward(self, src, tgt):
        src_emb = self.embedding(src).requires_grad_()
        tgt_emb = self.embedding(tgt).requires_grad_()

        src_emb = self.pos_encoder(src_emb).requires_grad_()
        tgt_emb = self.pos_encoder(tgt_emb).requires_grad_()

        for layer in self.encoder_layers:
            src_emb = checkpoint.checkpoint(layer, src_emb, use_reentrant=False)  # ✅ Apply checkpointing

        output = self.fc_out(src_emb).requires_grad_()
        return output
    
def check_for_stop():
    """Check if a stop file exists to halt training."""
    return os.path.exists("stop_training.txt")
    ...
    for input_ids, tot, tgt in batch:
        ...
        if combined_target.size(0) > fixed_length:
            ...
            combined_target = torch.cat(
                (combined_target, torch.full((pad_len,), pad_token_id, dtype=torch.long, device=device_cpu)),
                dim=0
            )
        ...
        if input_ids.size(0) > fixed_length:
            ...
            padded_input = torch.cat(
                (input_ids, torch.full((pad_len,), pad_token_id, dtype=torch.long, device=device_cpu)),
                dim=0
            )
        padded_inputs.append(padded_input)
    
    padded_inputs = torch.stack(padded_inputs).to(device_cpu)
    padded_targets = torch.stack(padded_targets).to(device_cpu)
...
class ChunkedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels_tot = self.pad_sequence(labels_tot, self.max_length // 2)

        return torch.tensor(input_ids, dtype=torch.long), torch.tensor(labels, dtype=torch.long), torch.tensor(labels_tot, dtype=torch.long)
...
def save_model(model, model_path="trained_model.pth"):
    """Saves the model before stopping."""
...
def training_loop(model, epochs, batch_size, genetic_algo_var, use_chunked_dataset, tokenized_data_path, tokenizer_path, seq_len, learning_rate, architecture):
        ...
        try:
            ...
            if use_chunked_dataset:
                ...
                labels_tot = torch.stack(labels_tot).to(device_cpu)
                ...
                if device == 'cpu':
                    ...
                    dataloader = DataLoader(
                        ...
                        batch_size=int(batch_size),
                        shuffle=True,
                        num_workers=8,    # Uses multiple CPU threads for data loading
                        prefetch_factor=1, # Prefetches data ahead of time
                        pin_memory=True,
                        collate_fn=collate_fn
                    )
            ...
            for epoch in range(epochs):
                if check_for_stop():  # ✅ Stop training early
                    print("🛑 Stop signal received. Saving model before exiting...")
                    save_model(model)  # ✅ Ensure model is saved
                    exit(0)  # ✅ Clean exit
                ...
                for batch_idx, (batch_input_ids, batch_labels) in enumerate(dataloader):
                    ...
                    with torch.amp.autocast('cuda', dtype=torch.float16):  # Enable mixed precision
                        ...
                        if use_genetic_algo == "Genetic Algorithm":
                            ...
                            try:
                                if architecture == "Rule Transformer":
                                    ...
                                    output = checkpoint.checkpoint(model, batch_input_ids, decoder_input, use_reentrant=False)
                        ...
                        if (batch_idx + 1) % accumulation_steps == 0:     
                                ...
                                for opt in optimizers:
                                    scaler.unscale_(opt)
                                    #torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0)  # ✅ Clip gradients for stability
...
if __name__ == "__main__":
    ...
    cleanup(model, "model_backup.pth")  # Cleanup after training
