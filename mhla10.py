...
class RuleTransform(nn.Module):
    def __init__(self,vocab_size, embedding_dim, max_rules, device):
        super().__init__()
        self.rule_transform = nn.Parameter(torch.randn(max_rules, embedding_dim, embedding_dim), requires_grad=False)
    ...
    def forward(self, hidden_states, token_ids):
        ...
        rule_indices = self.token_rules[token_ids]  # (batch, seq)
        rule_matrices = self.rule_transform[rule_indices]  # (batch, seq, d, d)
        rule_matrices = F.normalize(rule_matrices, p=2, dim=(-2, -1))
        return torch.einsum("bsd,bsde->bse", hidden_states, rule_matrices)
    ...
    def replace_rule(self):
        """
        Replace a low-scoring rule with a synthesized one.
        """
        k = min(self.max_rules, 10)
        bottom_k = torch.topk(self.rule_scores, k, largest=False).indices
        worst_rule_idx = bottom_k[torch.randint(0, k, (1,))].item()

        top_k = torch.topk(self.rule_scores, k, largest=True).indices
        new_rule = self.rule_transform[top_k].mean(dim=0).detach()
    ...
    def validate_and_replace_rule(self, model, val_input, val_target, decoder_input, loss_fn, k=5):
        """
        Uses local validation to evolve and evaluate rules before replacing weak ones.
        """
        worst_k = torch.topk(self.rule_scores, k, largest=False).indices

        top_k = torch.topk(self.rule_scores, k, largest=True).indices
        candidate_rule = self.rule_transform[top_k].mean(dim=0).detach()

        # Try replacing each of the worst rules and test performance
        best_loss = float('inf')
        best_idx = None

        for idx in worst_k:
            # Temporarily inject the candidate rule
            ...
            # Restore original before trying the next
            with torch.no_grad():
...
class Rule_Transformer_Model(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads, seq_length, lambda_decay=0.01, dropout=0.1, compression_factor=4, num_frequencies=100, max_rules=set_number_rules):
        ...
        self.rule_transform = RuleTransform(vocab_size, embedding_dim, max_rules, device=self.token_embedding.weight.device)
...
def build_custom_validation_batch(tokenizer, seq_len=1024, device='cuda', batch_size=4):
    query_strings = [
        "1. What is 17 + 35?",
    ...
    def training_loop(self):
        ...
        try:
            ...
            adamw_params = [
                ...
                or ("embedding" in name and "rule_transform" not in name)
                        ...
                        if self.use_genetic_algo == "Genetic Algorithm":
                            ...
                            if (batch_idx + 1) % accumulation_steps == 0:     
                                    ...
                                    if architecture == "Rule Transformer":
                                        ...
                                        self.model.rule_transform.validate_and_replace_rule(self.model, val_input, val_target, val_input, loss_fn)
