...
class DynamicRuleEmbedding(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_frequencies=50, max_rules=1000, device=device):
        super().__init__()
        self.vocab_size = vocab_size
        self.embedding_dim = embedding_dim
...
class Rule_Transformer_Model(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads, seq_length, lambda_decay=0.01, memory_size=5, dropout=0.1, compression_factor=2, num_frequencies=50, max_rules=1000):
...
class ReasoningModelGUI:
                        ...
                        with open(file_path, 'r', encoding='utf-8') as f:
                            if file.endswith('.jsonl'):
                                ...
                                if not self.query_target_pairs:
                                    # If reasoning separators are missing, fall back to user/assistant pairs
                                    self.query_target_pairs = [(m.get("content", ""), "", "") for m in conversation]                                
                                # After loading query_target_pairs
                                for i in range(min(5, len(self.query_target_pairs))):
                                    query, target, tot_target = self.query_target_pairs[i]
    ...
    def extract_query_target_pairs(self, messages, seq_len):
        query_target_pairs = []

        for i in range(len(messages) - 1):
            user_msg, assistant_msg = messages[i], messages[i + 1]
            
            if user_msg.get("role") in ["user", "human"] and assistant_msg.get("role") in ["assistant", "gpt"]:
                query = user_msg.get("content", "").strip()
                target = assistant_msg.get("content", "").strip()
                tot_target = self.extract_thinking(assistant_msg)
                query_target_pairs.append((query, target, tot_target))
